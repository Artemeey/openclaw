...
Eligible hybrid first attempts move the same two Windows jobs from Blacksmith to standard GitHub-hosted capacity: two fewer Blacksmith registrations and two hosted admissions per run, with no job or concurrency-cap increase. The conservative burst envelope below still counts both rows as Blacksmith, so this route only adds registration headroom. Hosted capacity is shared across workflows and does not guarantee immediate admission.
