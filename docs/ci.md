...
- Release repo E2E keeps gateway and agent-plugin checks together and runs the complete Control UI suite in four duration-weighted shards, including real-Gateway tests. These five jobs share a four-job concurrency cap and retain independent 90-minute budgets, so gateway coverage cannot consume the browser suite's time budget.
