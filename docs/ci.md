...
- **TUI PTY** runs two built-CLI artifact canaries in `build-artifacts`: a local model roundtrip and a real Gateway connection. The complete suite is defined in `test/vitest/vitest.tui-pty.config.ts`, but its `core-runtime-tui-pty` descriptor is not consumed by the CI matrix. Manual and release CI use the same canary selection; full-suite CI admission remains a coverage gap.
...
Run `33449014227` emitted 74 nondist compact rows, 72 plugin fallback rows and 79 other job records. QA/provider partitioning adds five fallback rows, and the Mac split adds two other rows. Conservatively count all 81 other records as Blacksmith admissions, including hosted and skipped records. This gives a broad-PR bound of `96 + 77 + 81 = 254` against the compact cap; canonical main has no PR fallback descriptors, giving a bound of 177. Two active main slots plus both pending successors and the previously observed peak of fourteen non-skipped PR arrivals, including superseded heads, give `4 × 177 + 14 × 254 = 4,264` registrations in five minutes. The live bucket rechecked on 2026-08-31 was 10,000; the 6,000 operating target leaves 1,736 for adjacent repositories and burst overlap. This is an arrival-envelope estimate, not an organization-wide admission cap: re-evaluate emitted rows, adjacent repositories and queued-run overlap before increasing fanout. An unused registration bucket does not prove physical runner capacity.
