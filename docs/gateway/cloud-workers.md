...
Archiving a non-main cloud-worker session with an active placement also performs this safe stop and reclaim before the Gateway records it as archived. If the placement is still transitioning or failed without proof that its environment is gone, the session remains unarchived and the request reports the blocking placement state. Wait while an active recovery is progressing. If a staged or prepared workspace result remains fenced, let recovery apply it; repeated archive requests do not discard it. Restoring the session retains the reclaimed placement metadata so the next turn can dispatch a fresh worker with the same workspace profile.

For a broken or runaway cloud environment, an administrator can call the admin-only `environments.destroy` method with `{ "environmentId": "<environment-id>", "force": true }` as a last resort. The client must warn that unreconciled workspace changes can be lost before sending this request. Forced teardown durably marks the placement failed and abandons any unreconciled remote result before destroying the environment.
