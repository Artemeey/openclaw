    ...
    profiles: {
      production: {
        ...
        settings: {
          ...
          ttl: "24h",
          idleTimeout: "60m",
          // Optional absolute path instead of a managed or PATH installation.
...
- `settings.class` (optional): provider-specific machine class passed to `--class`. Omission delegates sizing to Crabbox or the provider snapshot; explicit values are validated by Crabbox. Direct Daytona omits it and uses native snapshot sizing instead.
- `settings.ttl` and `settings.idleTimeout` (required): positive Go duration strings passed to `--ttl` and `--idle-timeout`. The selected backend owns lifetime and inactivity enforcement; idle stop may retain storage rather than destroy it. New guided profiles use `8h` and `45m`, plus Gateway-owned `suspendAfter: "15m"`; existing profile values are preserved.
- `settings.connectionId`: selects a named connection under `plugins.entries.crabbox.config.connections`. Its `provider` must match the profile backend; missing or unresolved credentials fail closed rather than falling back to ambient keys.
- `settings.organizationId`, `settings.snapshot`, and `settings.target`: bounded nonsecret native account/allocation choices for a named connection. Guided Daytona requires the intended organization ID and an explicit Linux snapshot. These settings stay in the durable profile snapshot used for lifecycle operations; rotating the credential does not rewrite the stored account scope.
- `settings.setup`: optional idempotent preparation command. The plugin subsequently prepares supported Node and the exact released OpenClaw runtime before enrollment. Unpublished builds need an exact candidate supplied by the image or this command. Keep cloud and model credentials out of setup.
- `settings.warmImage`: set to `true` to capture a reusable machine image at each stop and start later workers with the same profile from it; pair with `suspendAfter` so suspended sessions wake warm. Requires an effective configured or per-placement machine class before allocation. Disabled by default because images incur provider snapshot storage charges and retain whatever `setup` wrote outside the scrubbed worker state. Capture adds at most about three minutes to a stop on providers with slow native snapshots, then degrades to cold-only provisioning. Named connections currently reject warm images.
- `settings.binary`: optional absolute Crabbox executable path. Named connections otherwise use the verified managed installation or a compatible executable on `PATH`; they never select a sibling development checkout. Existing manual profiles retain their operator-managed executable lookup, including a sibling Crabbox binary in development checkouts. Settings installs the pinned CLI only after explicit confirmation; ordinary dispatch never installs it.

Unknown settings are rejected. Named connections store `label`, `provider`, and credential SecretRefs in the plugin's closed config schema. Guided API keys use protected entries in the existing SQLite secret store; the store is not encrypted at rest. The plugin resolves credentials into only the selected local Crabbox child's environment. Existing profiles without `connectionId` retain operator-managed Crabbox configuration. Do not put plaintext credentials in profile `settings`. See [guided cloud setup](/gateway/cloud-workers#connect-a-cloud-provider-in-settings) for the reference-only configuration example. Provisioning passes one deterministic canonical lease ID through `--lease-id`, keeps `--slug` as display metadata only, and always passes `--keep=true`; OpenClaw owns the external lifecycle and destroys the lease with `crabbox stop --id <canonical-id>`. After an ambiguous result, Gateway reconciliation repeats the same fixed-ID operation. Crabbox must return the exactly attested lease or fail closed; OpenClaw never falls back to slug adoption or replacement allocation.
