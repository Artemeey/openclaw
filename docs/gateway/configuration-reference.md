...
- `settings.ttl` and `settings.idleTimeout` (required): positive Go duration strings passed to `--ttl` and `--idle-timeout` as provider-side failsafes. New guided profiles use `8h` and `45m`, plus Gateway-owned `suspendAfter: "15m"`; existing profile values are preserved.
