...
or restart of the same run. Cron attempt completions remain `finishing` across
model fallbacks and interim-acknowledgment retries. Completed execution facts are
recorded before cron bookkeeping, so a later workflow error or abort cannot
reclassify that completed agent execution. A new attempt clears the prior outcome
before preparation; cron persistence and delivery retain their separate results.
