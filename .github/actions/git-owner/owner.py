...
def checkout():
    ...
    if sha == os.environ["WORKFLOW_SHA"]:
        # Checkout just populated this index from the workflow revision. Copy its
        # action files without updating the index or trusting later workspace edits.
        paths = git_output(workspace, "ls-files", "-z", "--", ".github/actions").split("\0")[:-1]
        run_git(workspace, "checkout-index", f"--prefix={harness}/", "--", *paths)
    else:
