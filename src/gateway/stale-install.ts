...
function resolveMissingInstallPath(error: Error): string | null {
  if (hasNodeErrorCode(error, "ENOENT")) {
    const syscall: unknown = Object.getOwnPropertyDescriptor(error, "syscall")?.value;
    const missingPath: unknown = Object.getOwnPropertyDescriptor(error, "path")?.value;
    return syscall === "open" && typeof missingPath === "string" && /\.[cm]?js$/u.test(missingPath)
      ? missingPath
      : null;
  }
...
export function classifyGatewayStaleInstall(error: unknown): GatewayStaleInstall | null {
  ...
  return {
    error: errorShape(
      ...
      {
        details: { code: GatewayErrorDetailCodes.STALE_INSTALL, restartCommand },
        retryable: false,
      },
    ),
    restartCommand,
