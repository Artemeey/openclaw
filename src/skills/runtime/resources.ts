...
type DeliveryCache = Map<string, SkillResourceDelivery | undefined>;
const deliveryCache = new WeakMap<SkillSnapshot, DeliveryCache>();
// Hydration recreates snapshot objects. The watcher-owned version and exact selection/paths
// retain immutable resource bytes across turns without filesystem freshness polling.
const versionedDeliveries: DeliveryCache = new Map();
...
export async function prepareSkillResourceDelivery(
  ...
  const cacheKey =
    snapshot.version === undefined
      ? JSON.stringify(explicitSelections ?? [])
      : JSON.stringify([
          snapshot.version,
          snapshot.prompt,
          snapshot.skillRoots,
  ...
  let cache = snapshot.version === undefined ? deliveryCache.get(snapshot) : versionedDeliveries;
  if (!cache) {
    cache = new Map();
    deliveryCache.set(snapshot, cache);
  }
  if (cache.has(cacheKey)) {
    return cache.get(cacheKey);
  }
  // Cache completed bytes, not a promise bound to another turn's authority.
  // Concurrent cold reads stay independent so cancelling one cannot fail the others.
  const result = await buildSkillResourceDelivery(snapshot, assertCurrent, explicitSelections);
  assertCurrent();
...
async function buildSkillResourceDelivery(
