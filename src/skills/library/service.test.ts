...
describe("profile-owned skill publication and selection", () => {
  ...
  it("delivers complete immutable supporting files into a worker-owned directory", async () => {
    ...
    try {
      ...
      await expect(
        prepareSkillResourceDelivery(structuredClone(snapshot), () => {
          throw new Error("closed turn");
        }),
      ).rejects.toThrow("closed turn");
      await expect(
        prepareSkillResourceDelivery({ ...snapshot, version: 2 }, () => {}),
      ).rejects.toThrow();
    } finally {
