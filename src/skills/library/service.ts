...
export async function readSkillLibrary(
  ...
  const prepared = prepareSkillLibraryBundle(files);
  if (prepared.revision !== result.entry.revision) {
    throw new SkillLibraryError(
      "INVALID_BUNDLE",
  ...
  return {
    ...
    content: prepared.files.find((file) => file.path === "SKILL.md")!.bytes.toString("utf8"),
