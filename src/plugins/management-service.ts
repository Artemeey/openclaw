...
function resolveCatalogPackageSourceIdentities(
  ...
  return resolveOfficialExternalPluginInstallSources(entry).flatMap((source) => {
    const packageName =
      source.source === "npm"
        ? parseRegistryNpmSpec(source.spec)?.name
        : parseClawHubPluginSpec(source.spec)?.name;
    return packageName ? [{ source: source.source, packageName }] : [];
  });
}
...
export const inspectManagedPlugin = withManagedPluginCache(
  async (params: {
    ...
    const manifest = getOfficialExternalPluginCatalogManifest(entry);
    const packageName = resolveCatalogPackageSourceIdentities(entry)[0]?.packageName;
    const selectedSource = resolveOfficialExternalPluginInstallSources(entry)[0];
