...
describe("secret target registry fast path", () => {
  ...
  it("replaces compiled targets when the same channel receives a new contract", () => {
    ...
    const originalLoad = loadBundledPluginPublicArtifactModuleSyncMock.getMockImplementation()!;
    let currentContract = contract("oldToken");
    loadBundledPluginPublicArtifactModuleSyncMock.mockImplementation((params) =>
    ...
    try {
      ...
      loadBundledPluginPublicArtifactModuleSyncMock.mockImplementation(originalLoad);
    }
  });
});
