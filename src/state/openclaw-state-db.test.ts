...
`);
  ...
  it.each(["runtime open", "doctor repair"] as const)(
    ...
    (migrationPath) => {
      ...
      const publicationStart = OPENCLAW_STATE_SCHEMA_SQL.indexOf(
        "CREATE TABLE IF NOT EXISTS github_publication_requests (",
      );
      const publicationEnd = OPENCLAW_STATE_SCHEMA_SQL.indexOf("\n) STRICT;", publicationStart);
      const previousPublicationSchema = OPENCLAW_STATE_SCHEMA_SQL.slice(
        publicationStart,
        publicationEnd + "\n) STRICT;".length,
      ).replace("  source_branch TEXT NOT NULL,\n", "");
      previous.exec(`DROP TABLE github_publication_requests; ${previousPublicationSchema}`);
      previous.exec(`
