...
export function createSessionTranscriptMessagePreparer(params: {
  ...
  return (
    ...
    let message = source;
    if (
      (!params.skipBeforeMessageWriteHooks &&
        getGlobalHookRunner()?.hasHooks("before_message_write")) ||
      params.prepareAssistantTranscriptMessage
    ) {
      const next = runAgentHarnessBeforeMessageWriteHook({ ...params, message });
      if (!next) {
        return null;
      }
  ...
  const beforeMessageWrite = (
    ...
    const preparedMessage =
      event.message.role === "user"
        ...
        : undefined;
    const prepared = prepareTranscriptMessage(event.message, sourceAppend, (message) =>
      restorePreparedUserTurnOperationalMetaForRuntime({
