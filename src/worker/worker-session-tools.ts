...
export function createWorkerSessionTools(client: WorkerSessionRpcClient): AnyAgentTool[] {
  return [
    ...
    },
    {
      ...
      name: "sessions_send",
      description: [
        "Send a message to an authorized parent, child, or sibling session on this Gateway, whether it runs on the Gateway, a paired device, or a cloud worker.",
        "Cross-tree and stale-incarnation targets are denied by the Gateway.",
        SESSIONS_SEND_RESULT_GUIDANCE,
        'Status "no_reply" is terminal; do not wait for another result.',
      ].join(" "),
      parameters: Type.Object({
