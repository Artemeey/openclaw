            ...
            if (codeModeTurn === 1) {
              writeResponsesEvents(
                ...
                buildToolCallEventsWithArgs("exec", {
                  code: `return await automations({ action: "update", jobId: ${JSON.stringify(codeModeJobId)}, job: { delivery: { mode: "announce", channel: "discord", to: "telegram:123" } } });`,
                }),
              );
              return;
            }
      ...
      try {
        ...
        await expect(
          client.request("cron.update", {
            id: added.id,
            patch: {
              delivery: { mode: "announce", channel: "discord", to: "telegram:123" },
            },
          }),
