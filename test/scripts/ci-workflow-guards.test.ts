...
`;
  ...
  it("keeps Night Clawer Telegram proof main-only, brokered, and sanitized", () => {
    ...
    expect(proof.run).toContain('--controller-gid "$CONTROLLER_GID"');
    expect(proof.run).toContain('--controller-uid "$CONTROLLER_UID"');
    ...
    const identity = expectDefined(
      steps.find((step) => step.name === "Create isolated Night Clawer controller identity"),
      "isolated controller identity step",
    );
    expect(identity.run).toContain("sudo useradd");
    expect(identity.run).toContain('controller_uid="$(id -u "$controller_user")"');
    expect(identity.run).toContain('[[ "$controller_uid" != "$(id -u)"');

    const cleanup = expectDefined(
      steps.find((step) => step.name === "Remove isolated Night Clawer controller identity"),
      "isolated controller cleanup step",
    );
    expect(cleanup.if).toBe("always()");
    expect(cleanup.run).toContain('sudo pkill -KILL -U "$CONTROLLER_UID"');
    expect(cleanup.run).toContain('! pgrep -U "$CONTROLLER_UID"');
    expect(cleanup.run).toContain('sudo userdel "$CONTROLLER_USER"');
