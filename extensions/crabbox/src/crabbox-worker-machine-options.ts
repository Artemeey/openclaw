...
type CrabboxMachineOptionsResolverDependencies = {
  ...
  ) => Promise<{ binary: string; runCommand: CrabboxCommandRunner }>;
...
export function createCrabboxMachineOptionsResolver(
  ...
  const loadMachineShapes = async (
    binary: string,
    runCommand: CrabboxCommandRunner,
  ): Promise<CrabboxMachineShapes> => {
    ...
    const result = await runCommand([binary, "providers", "--json"], {
  ...
  return async (profile) => {
    ...
    const { binary, runCommand } = await dependencies.resolveContext(parsed);
    ...
    if (!shapes) {
      shapes = loadMachineShapes(binary, runCommand).catch((error: unknown) => {
