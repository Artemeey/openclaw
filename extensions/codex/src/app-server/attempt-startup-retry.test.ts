...
async function createStartupFailureFixture(
  ...
  await fs.writeFile(
    ...
    [
      ...
      "function failSqliteInitialization() {",
      ...
      '} else if ((mode === "transient" && attempt === 1) || stillContended) {',
      "  failSqliteInitialization();",
      "} else {",
      ...
      // Synchronize persistent failure with registration, without a wall-clock race.
      // The transient case separately covers exit before registration finishes.
      '    if (mode === "persistent") {',
      "      failSqliteInitialization();",
      "      lines.close();",
