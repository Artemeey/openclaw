...
// Registration publishes metadata and synchronous hooks; auth and usage load on invocation.
const loadAnthropicAuth = createLazyRuntimeModule(() => import("./auth.runtime.js"));
const loadAnthropicUsage = createLazyRuntimeModule(() => import("./usage.js"));
...
export function buildAnthropicProvider(): ProviderPlugin {
  ...
  >[0];
  const loadApiKeyAuth = createLazyRuntimeModule(async () =>
    (await loadAnthropicAuth()).createProviderApiKeyAuthMethod(apiKeyAuthOptions),
  );
  return {
    id: providerId,
    label: "Anthropic",
    ...
    auth: [
      {
        ...
        run: createLazyRuntimeMethod(
          loadAnthropicAuth,
          (runtime) => runtime.runAnthropicCliMigration,
        ),
        runNonInteractive: createLazyRuntimeMethod(
          loadAnthropicAuth,
          (runtime) => runtime.runAnthropicCliMigrationNonInteractive,
        ),
      },
      {
        id: "setup-token",
        ...
        run: async (ctx) =>
          (await loadAnthropicAuth()).runAnthropicSetupTokenAuth(ctx, defaultAnthropicModel),
        validateNonInteractive: async (ctx) =>
          Boolean((await loadAnthropicAuth()).validateAnthropicSetupTokenNonInteractive(ctx)),
        runNonInteractive: async (ctx) =>
          (await loadAnthropicAuth()).runAnthropicSetupTokenNonInteractive(
            ctx,
            defaultAnthropicModel,
          ),
      },
      {
        id: apiKeyAuthOptions.methodId,
        label: apiKeyAuthOptions.label,
        hint: apiKeyAuthOptions.hint,
        kind: "api_key",
        starterModel: apiKeyAuthOptions.defaultModel,
        wizard: apiKeyAuthOptions.wizard,
        run: createLazyRuntimeMethod(loadApiKeyAuth, (method) => method.run),
        validateNonInteractive: createLazyRuntimeMethod(
          loadApiKeyAuth,
          (method) => method.validateNonInteractive!,
        ),
        runNonInteractive: createLazyRuntimeMethod(
          loadApiKeyAuth,
          (method) => method.runNonInteractive!,
        ),
      },
    ],
    catalog: {
    ...
    resolveUsageAuth: createLazyRuntimeMethod(
      loadAnthropicUsage,
      (runtime) => runtime.resolveAnthropicUsageAuth,
    ),
    fetchUsageSnapshot: createLazyRuntimeMethod(
      loadAnthropicUsage,
      (runtime) => runtime.fetchAnthropicUsage,
    ),
    isCacheTtlEligible: () => true,
    buildAuthDoctorHint: createLazyRuntimeMethod(
      loadAnthropicAuth,
      (runtime) => runtime.buildAnthropicAuthDoctorHint,
    ),
  };
}
