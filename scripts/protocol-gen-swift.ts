...
// These properties shipped as AnyCodable before nullable arrays gained a
// concrete Swift type. Preserve that public source API for existing clients.
const LEGACY_ANYCODABLE_PROPERTIES = new Set([
  "SessionsPatchParams.inheritedToolAllow",
  "SessionsPatchParams.inheritedToolDeny",
  "SessionsPatchMutation.inheritedToolAllow",
  "SessionsPatchMutation.inheritedToolDeny",
]);
...
function swiftLegacyPropertyType(structName: string, key: string): string | undefined {
  return LEGACY_ANYCODABLE_PROPERTIES.has(`${structName}.${key}`) ? "AnyCodable?" : undefined;
}

function usesTypedNullableArray(structName: string, key: string, schema: JsonSchema): boolean {
  return !swiftLegacyPropertyType(structName, key) && unwrapNullableSchema(schema).nullable;
}
...
function unwrapNullableSchema(schema: JsonSchema): {
  schema: JsonSchema;
  nullable: boolean;
} {
  ...
  return valueVariants.length === 1 && valueVariants[0]?.type === "array"
    ? { schema: valueVariants[0], nullable: true }
...
function swiftType(schema: JsonSchema, required: boolean, allowStructuralNamed = false): string {
  ...
  const resolvedSchema = unwrapped.schema;
  const t = resolvedSchema.type;
  let base: string;
  const named = namedSchema(resolvedSchema, allowStructuralNamed);
  if (named) {
    base = named;
  } else if (t === "string") {
    base = "String";
    ...
    base = `[${swiftType(resolvedSchema.items ?? { type: "Any" }, true, true)}]`;
  } else if (resolvedSchema.enum) {
    base = "String";
  } else if (resolvedSchema.patternProperties) {
  ...
  return `${base}${"?".repeat(Number(!required) + Number(unwrapped.nullable))}`;
}
...
function emitStruct(name: string, schema: JsonSchema): string {
  ...
  for (const [key, propSchema] of Object.entries(props)) {
    ...
    const propType =
      swiftLegacyPropertyType(name, key) ?? swiftType(propSchema, required.has(key), true);
    lines.push(`    public let ${propName}: ${propType}`);
    ...
    "\n    public init(\n" +
      Object.entries(props)
        .map(([key, prop]) => {
          ...
          const legacyType = swiftLegacyPropertyType(name, key);
          if (legacyType) {
            return `        ${propName}: ${legacyType} = nil`;
          }
          return `        ${swiftInitializerParam({
            name: propName,
            schema: prop,
...
function emitStructCustomCodable(
  ...
  const preservesNullableArray = Object.entries(props).some(([key, propSchema]) =>
    usesTypedNullableArray(name, key, propSchema),
  );
  const preservesOptionalNull = name === "AgentsUpdateParams" && props.model;
  if (!preservesNullableArray && !preservesOptionalNull) {
    return "";
  }
  const decodedProperties = Object.entries(props).map(([key, propSchema]) => {
    ...
    if (usesTypedNullableArray(name, key, propSchema)) {
      return `        self.${propName} = container.contains(.${propName})\n            ? .some(try container.decode(${swiftType(propSchema, true, true)}.self, forKey: .${propName}))\n            : nil`;
    }
    return `        self.${propName} = try container.decodeIfPresent(${swiftType(propSchema, true, true)}.self, forKey: .${propName})`;
  });
