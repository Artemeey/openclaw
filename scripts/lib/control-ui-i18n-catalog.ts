...
export async function loadControlUiSourceCatalog(
  ...
  cloudWorkersSourceLocalePath: string,
  ...
  const cloudWorkersSource = (
    await importControlUiLocaleModule<{
      registerCloudWorkersEnglish: { catalog: TranslationMap };
    }>(cloudWorkersSourceLocalePath)
  ).registerCloudWorkersEnglish.catalog;
  if (!source || !activitySource || !sessionPlacementSource || !cloudWorkersSource) {
