...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 2) {
    ...
    if (key === "--package-root") packageRoot = value;
...
try {
  ...
  if (mode === "require") {
    assert.equal(
      ...
      1,
      "expected exactly one fs-safe platform package",
    );
    assert.equal(
    ...
    assert.equal(
      fs.realpathSync(path.dirname(loadedNativeModules[0])),
      installedPlatformPackages[0].root,
    );
  } else {
    assert.equal(
      installedPlatformPackages.length,
