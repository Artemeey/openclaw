...
final class OnboardingAISetupModel {
    ...
    var activeAuthOption: AuthOption?
    var providerWizardKind: ProviderWizardKind?
    var authStep: WizardStep?
    var authError: Failure?
    var authBusy = false {
        didSet { self.updateBusyReason() }
    }
    ...
    var statuses: [String: CandidateStatus] = [:]
    ...
    private(set) var attemptToken = UUID()
    ...
    @ObservationIgnored var serverLease: GatewayConnection.ServerLease?
    @ObservationIgnored private(set) var lastDetectedActivationState: PersistedActivationState?
    @ObservationIgnored var authSessionID: String?
    @ObservationIgnored var authAttemptID = UUID()
    /// Only a just-completed provider flow may trust setupComplete without re-probing.
    @ObservationIgnored var providerAuthReconciliationPending = false
...
extension OnboardingAISetupModel {
    ...
    func failDetectionForMissingRoute() {
        self.phase = .ready
        self.detectError = Self.transportFailure(
